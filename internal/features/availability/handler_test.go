...
func TestHandlerUnavailableReturnsServiceUnavailable(t *testing.T) {
	cause := errors.New("database connect: dial tcp 127.0.0.1:5432: connect: connection refused")
	h := NewHandler(func(context.Context) error { return cause }, cause, "")
	c, _ := testutil.NewRequestContext(http.MethodGet, "/", nil)
...
func TestHandlerUnavailableUsesSchemaMessage(t *testing.T) {
...
func TestHandlerHealthReportsStatus(t *testing.T) {
	tests := []struct {
		name        string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			h := NewHandler(tc.checkHealth, nil, "")
			c, rec := testutil.NewRequestContext(http.MethodGet, "/health", nil)
...
func TestHandlerHealthIncludesVersion(t *testing.T) {
	h := NewHandler(func(context.Context) error { return nil }, nil, "v1.2.3")
	c, rec := testutil.NewRequestContext(http.MethodGet, "/health", nil)
