...
func TestSessionManagerUserAndFlashRoundTrip(t *testing.T) {
	...
	rec = httptest.NewRecorder()
	if err := manager.SetFlash(rec, req, "notice", "saved"); err != nil {
	...
	rec = httptest.NewRecorder()
	flashes, err := manager.GetFlashes(req, rec, "notice")
	if err != nil || len(flashes) != 1 || flashes[0] != "saved" {
		t.Fatalf("GetFlashes() flashes=%#v err=%v", flashes, err)
	}
...
func TestSessionManagerPropagatesStoreErrors(t *testing.T) {
	...
	if _, err := manager.GetFlashes(req, rec, "notice"); err == nil {
		t.Fatal("GetFlashes() unexpectedly succeeded")
	}
	if err := manager.Clear(rec, req); err == nil {
