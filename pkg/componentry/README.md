...
`github.com/go-sum/componentry` is a [Gomponents]-based server-rendered UI component library built on a strict three-tier dependency architecture. Components are inspired by the [shadcn/ui] design system conventions using [Tailwind], render native HTML-first interactivity (no JavaScript required for most interactions), and integrate seamlessly with [HTMX] for progressive enhancement.

## Features

- Three-tier dependency DAG enforcing strictly downward imports between component layers
...
- Full [HTMX] integration with request/response header helpers, partial rendering, and smart redirects
...
- Flash messages via short-lived HTTP-only cookies
...
| **2** | `patterns/*`, `interactive/*` (tabs, dialog, dropdown, etc.) | Tier 0 + Tier 1 + stdlib/external |
...
| `interactive/*` | `github.com/go-sum/componentry/interactive/...` | Accordion, breadcrumb, dialog, dropdown, pagination, tabs, theme, tooltip |
| `patterns/*` | `github.com/go-sum/componentry/patterns/...` | Flash, font, form binding, head, htmx, pager, redirect |
...
| `ui/layout` | `github.com/go-sum/componentry/ui/layout` | Navbar, Sidebar |
...
assets.RegisterSprites(map[string]string{
    "lucide-icons": "sprites/lucide.svg",
})
assets.SetPathFunc(assets.Path)

url := assets.SpritePath("lucide-icons")
// => "/public/sprites/lucide.svg?v=abcd1234"
```
...
const ScriptCSPHash string
```

`ThemeScript` emits a synchronous `<script>` block that reads `localStorage['themePreference']` and sets the `data-theme-preference` attribute on `<html>` before any content renders. Place it at the top of `<head>`. Add `ScriptCSPHash` to your `Content-Security-Policy` `script-src` directive.

`ThemeSelector` renders an icon button that cycles through light, dark, and system modes.
...
Renders a `<button>` element. When `Href` is set, renders an `<a>` styled as a button instead.
...
Renders `<svg><use href="Src#ID"/></svg>`. `Size` defaults to `"size-4"`. When `Label` is empty, the icon is marked `aria-hidden="true"`.
...
```go
type LabelProps struct { For string; Error bool; Extra []g.Node }
...
type PopoverRootProps struct { ID string; Extra []g.Node }
type PopoverTriggerProps struct { Extra []g.Node }
type PopoverContentProps struct { Width, Align string; Extra []g.Node }
...
`Align` accepts `"left"`, `"right"`, or `"center"`.

#### Separator

```go
type SeparatorProps struct { ID, Label, Decoration string }

func Separator(p SeparatorProps) g.Node
```

`Decoration` accepts `"plain"` or `"dashed"`.
...
Table.Body(props TableBodyProps, children ...g.Node) g.Node
Table.Row(props TableRowProps, children ...g.Node) g.Node
...
data.Card.Root(
    ...
    data.Card.Content(
        data.Table.Root(
            data.Table.Header(
                data.Table.Row(data.TableRowProps{},
                    data.Table.Head(g.Text("Name")),
                    data.Table.Head(g.Text("Email")),
                ),
            ),
            data.Table.Body(data.TableBodyProps{},
                data.Table.Row(data.TableRowProps{},
...
Alert.List(types []AlertVariant, texts []string) g.Node
```
...
const ProgressDefault, ProgressSuccess, ProgressDanger ProgressVariant = ...

type ProgressSize string
const ProgressSizeDefault, ProgressSizeSm ProgressSize = ...
...
Responsive navigation bar with dropdowns on desktop and an accordion drawer on mobile.

**Item types** -- `NavbarItem` is a sealed interface. Only these types implement it:

| Type | Description |
|------|-------------|
| `NavLink` | Anchor link with optional icon |
...
| `NavForm` | Inline form (e.g., sign-out button) |
...
Modal drawer that closes on overlay click.

```go
type SidebarProps struct { ID string; Nav g.Node }
...
func ClickTrigger(extra []g.Node, children ...g.Node) g.Node
...
for _, msg := range msgs {
    // render feedback.Toast or feedback.Alert
}
```
...
`Submit` binds request data into `dest` and validates it. It never returns an error -- validation failures are stored internally and accessed via `IsValid`, `FieldHasErrors`, and `GetFieldErrors`. Struct-level failures are stored under the `"_"` key.
...
```go
type MetaProps struct { Title, Description, Keywords, FaviconHref, OGImage string }
...
`Head` renders the complete `<head>` element. `Metatags`, `CSS`, and `JS` render individual sections for use in custom head compositions.

---

### patterns/htmx

HTMX request header extraction and response header helpers.
...
type Response struct {
    Redirect, Refresh, PushURL, ReplaceURL                string
...
`New` creates isolated registries (useful for tests). `ApplyDefault` populates the package-level `Default` registries in `assets` and `icons`. Call `ApplyDefault` once at startup before handling any requests.
...
Mount at a development route (e.g., `/components`) to provide an in-app component reference:
...
- `patterns/htmx` extracts HTMX request headers and sets response headers
